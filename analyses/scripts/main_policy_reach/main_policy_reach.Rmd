---
title: "Effect of the Policy Reach"
...
# plot specifications ---
SAVE <- TRUE
EXPERIMENT_PREFIX <- "main_policyreach"
RESOLUTION_DPI <- 300
THEME_DESIGN <- 2
plot_no <- 0
...
The factor `PublicSpaces` has been sampled from [0, 100] in steps of 5, yielding 21 different configurations. 10 replicates have been run for each configuration.
...
height <- 5
labs <- c(
  title = "Effect of policy reach on epidemic peak",
  x = "Number of public spaces", 
...
# plotting ---
summary_statistics_aggregated %>% 
  ggplot(aes(x = PublicSpaces, y = PeakInfectiveFractionMean)) +
    geom_line() +
    geom_point(size = 2) +
    geom_errorbar(
      aes(
    ...
    scale_y_continuous(breaks = seq(0, 1, by = 0.05), limits = c(0.45, 1.0)) +
...
height <- 5
labs <- c(
  title = "Effect of policy reach on step of epidemic peak",
  x = "Number of public spaces", 
...
# plotting ---
summary_statistics_aggregated %>% 
  ggplot(aes(x = PublicSpaces, y = TickOfPeakInfectiveCountMean)) +
    geom_line() +
    geom_point(size = 2) +
    geom_errorbar(
      aes(
...
height <- 5
labs <- c(
  title = "Effect of policy reach on duration of epidemic",
  x = "Number of public spaces", 
...
# plotting ---
summary_statistics_aggregated %>% 
  ggplot(aes(x = PublicSpaces, y = DurationOfEpidemicMean)) +
  geom_line() +
  geom_point(size = 2) +
  geom_errorbar(
    aes(
...
height <- 5
labs <- c(
  title = "Effect of policy reach on fraction of remaining susceptibles",
  x = "Number of public spaces", 
...
# plotting ---
summary_statistics_aggregated %>% 
  ggplot(aes(x = PublicSpaces, y = FractionStillSusceptibleMean)) +
    geom_line() +
    geom_point(size = 2) +
    geom_errorbar(
      aes(
...
summary_statistics_aggregated %>% 
  ggplot(aes(x = PublicSpaces, y = PeakInfectiveFractionMean)) +
    ...
    scale_x_continuous(breaks = seq(0, 100, by = 5)) +
    scale_y_continuous(breaks = seq(0, 1, by = 0.1), limits = c(0.45, 1.0)) +
    labs(
...
# plotting ---
summary_statistics %>% 
  ggplot(aes(x = PublicSpaces, y = PeakInfectiveFraction)) +
...
# plotting ---
summary_statistics %>% 
  ggplot(aes(x = PublicSpaces, y = TickOfPeakInfectiveCount)) +
    geom_point(size = 2, alpha = 0.2) +
    scale_x_continuous(breaks = seq(0, 100, by = 5)) +
    scale_y_continuous(breaks = seq(0, 1000, by = 50), limits = c(150, 500)) +
    labs(title = labs["title"], x = labs["x"], y = labs["y"]) +
    theme_thesis(plot_margin = 15, design = THEME_DESIGN) +
    NULL
...
# plotting ---
summary_statistics %>% 
  ggplot(aes(x = PublicSpaces, y = DurationOfEpidemic)) +
  geom_point(size = 2, alpha = 0.2) +
  scale_x_continuous(breaks = seq(0, 100, by = 5)) +
...
# plotting ---
summary_statistics %>% 
  ggplot(aes(x = PublicSpaces, y = FractionStillSusceptible)) +
    geom_point(size = 2, alpha = 0.2) +
    scale_x_continuous(breaks = seq(0, 100, by = 5)) +
    scale_y_continuous(breaks = seq(0, 1, by = 0.005)) +
    labs(title = labs["title"], x = labs["x"], y = labs["y"]) +
...
## Epidemic Curves by Policy Reach
...
mdata_summarized_long %>% 
  ...
  right_join(config %>% select(Config, PublicSpaces), by = "Config") %>% 
  filter(PublicSpaces <= 50) %>% 
  group_by(PublicSpaces, Step) %>% 
  summarize(MeanFraction = mean(Fraction)) %>% 
  ungroup() -> tmp1

tmp1$PublicSpaces %<>% factor(levels = seq(0, 50, by = 5))
...
tmp2$PublicSpaces %<>% factor(levels = seq(0, 50, by = 5))

# plotting ---
tmp1 %>% 
  ggplot(aes(x = Step, y = MeanFraction, color = PublicSpaces)) +
    ...
    guides(color = guide_legend(title = "Public spaces", reverse = TRUE)) +
    theme_thesis(plot_margin = 15, design = THEME_DESIGN) +
...
labs <- c(
  ...
  x = "Day", 
  y = "Fraction of agents",
  caption = "New cases per day split by policy reach (aggregated over all replicates).\nThe blue dashed lines indicate the start of the policy."
)

# data formatting ---
mdata_long %>% 
  inner_join(config %>% select(Config, PublicSpaces), by = "Config") %>% 
  select(-Config, -Step) %>% 
  filter(Concept == "NewCasesReal") %>% 
  group_by(PublicSpaces, Replicate, Day, Concept) %>%
  summarize(Fraction = sum(Fraction)) %>%
  ungroup() %>% 
  group_by(PublicSpaces, Day) %>% 
  summarize(Fraction = mean(Fraction)) %>% 
  ungroup() %>% 
  filter(PublicSpaces <= 50 & PublicSpaces %% 10 == 0) -> tmp

# plotting ---
tmp %>% 
  ggplot(aes(x = Day)) +
    geom_bar(aes(y = Fraction), stat = "identity", width = 0.5) +
    geom_vline(xintercept = 10, color = "royalblue1", linetype = "dashed", size = 0.7) +
    facet_wrap(. ~ PublicSpaces, ncol = 1, strip.position = "right") +
