...
library(igraph)
library(viridis)
EXPERIMENT_NAME <- "small_scale_detail"
...
# read and format agent attributes ---
dir <- file.path("..", "..", "..", "experiments", EXPERIMENT_NAME)
archive_filename <- file.path(dir, "agent_attributes.7z")
unpack_7z_command <- paste0("7z x ", archive_filename)
system(unpack_7z_command)
agent_attributes <- data.frame()
for (config_key in 1:dim(config)[1]) {
  config_nr <- stringr::str_pad(config_key, 2, "left", "0")
  filename <- paste0("config_", config_nr, "_agent_attributes.feather")
  df <- arrow::read_feather(filename)
  df$Config <- paste0("config_", config_nr)
  agent_attributes %<>% bind_rows(df)
}
system("rm *.feather")
agent_attributes %<>% 
  mutate(
    ...
    TrustAuthorities = as.numeric(TrustAuthorities),
    IncubationPeriod = as.numeric(IncubationPeriod),
    InfectionDuration = as.numeric(InfectionDuration)
  )

# read and format agent dynamics ---
dir <- file.path("..", "..", "..", "experiments", EXPERIMENT_NAME)
archive_filename <- file.path(dir, "agent_dynamics.7z")
unpack_7z_command <- paste0("7z x ", archive_filename)
system(unpack_7z_command)
agent_dynamics <- data.frame()
for (config_key in 1:dim(config)[1]) {
  config_nr <- stringr::str_pad(config_key, 2, "left", "0")
  filename <- paste0("config_", config_nr, "_agent_dynamics.feather")
  df <- arrow::read_feather(filename)
  df$Config <- paste0("config_", config_nr)
  agent_dynamics %<>% bind_rows(df)
}
system("rm *.feather")
agent_dynamics %<>%
  mutate(
    ...
    Socialnorm = as.numeric(Socialnorm),
    Behavior = as.logical(Behavior),
    Quarantined = as.logical(Quarantined),
    State = as.factor(State)
  )

# read and summarize graphs ---
size_graph_complete <- unique(config$Nodes) + unique(config$AgentCount)
size_interaction_space <- unique(config$Nodes)
n_replicates <- dim(mdata %>% select(Replicate) %>% unique())[1]
dir <- file.path("..", "..", "..", "experiments", EXPERIMENT_NAME)
archive_filename <- file.path(dir, "graphs.7z")
unpack_7z_command <- paste0("7z x ", archive_filename)
system(unpack_7z_command)
summary_graphs <- data.frame()
for (config_key in 1:dim(config)[1]) {
  config_nr <- stringr::str_pad(config_key, 2, "left", "0")
  for (rep_key in 1:n_replicates) {
    rep_nr <- stringr::str_pad(rep_key, 2, "left", "0")
    filename <- paste0("config_", config_nr, "_rep_", rep_nr, "_graph.txt")
    edgelist <- read_delim(filename, delim = ",", col_names = FALSE)
    g <- igraph::graph_from_data_frame(edgelist)
    g <- g - c((size_interaction_space + 1):size_graph_complete)
    df <- degree(g) %>% as.data.frame(row.names = NULL)
    names(df) <- c("Degree")
    df$Config <- paste0("config_", config_nr)
    df$Replicate <- rep_key
    df$Pos <- 1:length(V(g))
    df %<>% arrange(desc(Degree)) %>% mutate(Rank = 1:dim(df)[1])
    summary_graphs %<>% bind_rows(df)
  }
}
system("rm *_graph.txt")
summary_graphs %<>%
  mutate(
    Config = as.factor(Config),
    Replicate = as.numeric(Replicate),
...
save(
  list = c(
    "agent_attributes", "agent_dynamics", "config", "mdata", "mdata_long", 
    "mdata_summarized", "mdata_summarized_long", "summary_agent_states", 
    "summary_graphs", "summary_statistics", "summary_statistics_aggregated"
  ),
  file = file.path("data.RData")
)
rm(
  archive_filename, config_key, config_nr, df, dir, edgelist, filename, g, n_replicates, 
  rep_key, rep_nr, size_graph_complete, size_interaction_space, unpack_7z_command
)